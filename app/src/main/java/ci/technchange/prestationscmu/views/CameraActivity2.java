...
public class CameraActivity2 extends AppCompatActivity {
    ...
    /*private void processImageAndNavigate(File photoFile) {
        try {
            ...
            intent.putExtra("NUM_SECU", numSecu);
            intent.putExtra("GUID", guid);
            intent.putExtra("AFFECTION1", affection1);
            intent.putExtra("AFFECTION2", affection2);
            intent.putExtra("ACTE1", acte1);
            intent.putExtra("ACTE2", acte2);
            intent.putExtra("ACTE3", acte3);
            startActivity(intent);
            finish(); // Fermer cette activité
        } catch (Exception e) {
            Log.e(TAG, "Erreur lors du traitement de l'image: " + e.getMessage());

            // En cas d'erreur, passer directement à l'écran de prévisualisation
            Intent intent = new Intent(this, PreviewActivity.class);
            intent.putExtra("imagePath", photoFile.getAbsolutePath());
            intent.putExtra("NUM_TRANS", numTrans);
            intent.putExtra("NUM_SECU", numSecu);
    ...
    // Dans processImageAndNavigate() de CameraActivity
    private void processImageAndNavigate(File photoFile) {
        try {
            // Au lieu d'ouvrir PreviewActivity, retourner directement à l'activité appelante
            Intent returnIntent = new Intent();
            returnIntent.putExtra("imagePath", photoFile.getAbsolutePath());
            returnIntent.putExtra("NUM_TRANS", numTrans);
            setResult(Activity.RESULT_OK, returnIntent);
            finish();
        } catch (Exception e) {
            Log.e(TAG, "Erreur lors du traitement: " + e.getMessage());
            // En cas d'erreur, renvoyer quand même les données
            Intent returnIntent = new Intent();
            returnIntent.putExtra("imagePath", photoFile.getAbsolutePath());
            returnIntent.putExtra("NUM_TRANS", numTrans);
            setResult(Activity.RESULT_OK, returnIntent);
